...
class InstalTypeChecker(TypeChecker):
    ...
    def check_types(self):
        ...
        self.check_initials(self.IR_dict["initials"])
        self.check_bridge()

    def check_initials(self, initials):
        #TODO: Include a arg checker
        for i in initials:
            self.check_term_declared(
                ["fluents", "powers", "permissions", "obligation_fluents"], i[0])
            self.is_valid_condition(i[1])
    ...
    def get_term_declared(self, keys_to_check, term):
        ...
        if term[0] == "perm":
            ...
            if "powers" in keys_to_check:
                declared_term = self.get_term_declared(["exevents"], term[1]) # TODO: change to exevents
                if declared_term:
            ...
            raise NotImplementedError("I'm not sure if you can do this?") # TODO: what?
        else:
            keys_to_check = [
    ...
    def check_term_declared(self, keys_to_check, term):
        # TODO: Add an error step which tells you if something hasn't been declared at all or is listed as the wrong thing
